...
contract AcPool is IAcPool, Ownable {
    ...
    /// @param acLpAmount Amount of LP tokens to burn
    ...
    function unstakeAndWithdraw(uint256 acLpAmount, uint256 minUnderlyingReceived)
        external
        returns (uint256)
    {
        controller.lpTokenStaker().unstakeFrom(acLpAmount, msg.sender);
        return withdraw(acLpAmount, minUnderlyingReceived);
    }
    ...
    /// by another user
    /// @return uint256 Total underlying withdrawn
    function withdraw(uint256 acLpAmount, uint256 minUnderlyingReceived)
        public
        override
        returns (uint256)
    {
        // Preparing Withdrawals
        require(lpToken.balanceOf(msg.sender) >= acLpAmount, "insufficient balance");
        ...
        uint256 underlyingToReceive_ = acLpAmount.mulDown(_exchangeRate(totalUnderlying_));
        {
        ...
        lpToken.burn(msg.sender, acLpAmount);
