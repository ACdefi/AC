    ...
    BaseMinter
{
    ...
    function handleRebalancingRewards(
        ...
        _distributeRebalancingRewards(address(acPool), account, cncRewardAmount);
    }
    ...
    function computeRebalancingRewards(
        ...
        uint256 cncPerSecond = poolCNCRebalancingRewardPerSecond(acPool);
        uint256 deviationDelta = deviationBefore - deviationAfter;
        uint256 deviationImprovementRatio = deviationDelta.divDown(
            IAcPool(acPool).totalDeviationAfterWeightUpdate()
        );
        uint256 lastWeightUpdate = controller.lastWeightUpdate(acPool);
