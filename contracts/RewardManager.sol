...
contract RewardManager is IRewardManager, Ownable {
    ...
    {
        address[] memory curvePools = IAcPool(pool).allCurvePools();
    ...
    /// @dev Ac pool LP tokens need to be staked in the `LpTokenStaker` in
    ...
    function _claimPoolEarnings() internal {
        ...
        IConvexHandler(convexHandler).claimBatchEarnings(IAcPool(pool).allCurvePools(), pool);
    ...
    function addExtraReward(address reward) public override onlyOwner returns (bool) {
        ...
        address[] memory curvePools_ = IAcPool(pool).allCurvePools();
