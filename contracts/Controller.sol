...
contract Controller is IController, Ownable {
    ...
    function shutdownPool(address poolAddress) external override onlyOwner {
        ...
        IAcPool(poolAddress).shutdownPool();
    ...
    function updateWeights(WeightUpdate memory update) public override onlyOwner {
        require(
            lastWeightUpdate[update.acPoolAddress] + weightUpdateMinDelay < block.timestamp,
            "weight update delay not elapsed"
        );
        IAcPool(update.acPoolAddress).updateWeights(update.weights);
        lastWeightUpdate[update.acPoolAddress] = block.timestamp;
    }
